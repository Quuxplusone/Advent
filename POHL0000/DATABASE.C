...
int	val;
{
	...
	if ((v1 = binary(word, wc)) >= 0) {
		if (v1 > 0 && !strcmp(word, wc[v1 - 1].aword))
			v2 = v1 - 1;
		else if (v1 < MAXWC-1 && !strcmp(word, wc[v1 + 1].aword))
			v2 = v1 + 1;
		else
			v2 = v1;
		if (!val)
...
int	atloc;
{
	return(cond[atloc] == 2 || cond[atloc] == 3);
}
